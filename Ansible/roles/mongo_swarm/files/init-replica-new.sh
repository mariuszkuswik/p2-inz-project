...
RS_CONFIG=(["control-plane"]="27017"  ["node1"]="27018" ["node2"]="27019") 

DEFAULT_MAIN_NODE="control-plane"


# Function to initialize the replica set
initialize_replica_set() {
  echo "Initializing replica set"
  mongosh --host ${PRIMARY_NODE}:${PRIMARY_PORT} --eval "rs.initiate($RS_CONFIG)"
}
...
find_primary_node() {
  for node in RS_CONFIG; do
    IS_REPLICA_SET=`mongosh $node:${RS_CONFIG[$node]} --eval "rs.status()"`
